describe('Search', () => {
    ...
    it('should accept input', () => {
        ...
        cy.get('.search__input').type(inputText).should('have.value', inputText);
    });
    ...
    it('should click first search suggest result and scroll to relevant card', () => {
        cy.get('.card__header-name').contains('/books').should('not.be.visible');
        cy.get('.search__input').type('/books');
        cy.get('.search__suggest li:first').click();

        cy.wait(1000);

        cy.get('.card__header-name').contains('/books').should('be.visible');
    });
